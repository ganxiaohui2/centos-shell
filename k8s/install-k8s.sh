...
if [ $? -eq 0 ];then
	echo "================================================="
	echo "         etch安装成功，请按任意键继续！！        "
	echo "================================================="
	char=`get_char`
else
	echo "================================================="
	echo "         etch安装失败，请按任意键退出！！        "
	echo "================================================="
	char=`get_char`
	exit 1
fi
...
echo >>/opt/kubernetes/cfg/kube-apiserver<< EOF 
# 启用日志标准错误
...
EOF
echo >>/lib/systemd/system/kube-apiserver.service<< EOF 
[Unit]
...
echo >>/opt/kubernetes/cfg/kube-scheduler<< EOF 
===============================================================================
KUBE_LOGTOSTDERR="--logtostderr=true"
KUBE_LOG_LEVEL="--v=4"
...
echo >>/lib/systemd/system/kube-scheduler.service<< EOF 
[Unit]
...
echo >>/opt/kubernetes/cfg/kube-controller-manager<< EOF 
KUBE_LOGTOSTDERR="--logtostderr=true"
KUBE_LOG_LEVEL="--v=4"
KUBE_MASTER="--master=$IPADDR_MASTER:8080"
EOF
echo >>/lib/systemd/system/kube-controller-manager.service<< EOF 
[Unit]
...
ps aux | grep kube | grep -v grep | wc -l
if [ $? -eq 3 ];then
	echo "========================================================="
...
echo >>/opt/kubernetes/cfg/kubelet.kubeconfig<< EOF 
apiVersion: v1
kind: Config
clusters:
  - cluster:
      server: http://$IPADDR_7:8080
    name: local
contexts:
  - context:
      cluster: local
...
EOF
echo >>/opt/kubernetes/cfg/kubelet << EOF 
# 启用日志标准错误
KUBE_LOGTOSTDERR="--logtostderr=true"
# 日志级别
KUBE_LOG_LEVEL="--v=4"
...
echo >>/lib/systemd/system/kubelet.service<< EOF 
[Unit]
...
echo >>/opt/kubernetes/cfg/kube-proxy<< EOF 
# 启用日志标准错误
KUBE_LOGTOSTDERR="--logtostderr=true"
# 日志级别
KUBE_LOG_LEVEL="--v=4"
...
echo >>/lib/systemd/system/kube-proxy.service<< EOF 
[Unit]
